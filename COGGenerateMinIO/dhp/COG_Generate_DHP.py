...
def SinusoidalProject_to_epsg4326(input_file, output_file):
    # 打开原始GeoTIFF文件
    dataset = gdal.Open(input_file)

    # 设置转换选项，指定目标坐标系为EPSG:4326
    # 这里的"-t_srs EPSG:4326"表示目标坐标系
    # "-of GTiff"表示输出格式为GeoTIFF
    # "data_type"设置为与原始数据集相同，以保持数据类型不变

    warp_options = gdal.WarpOptions(format='GTiff',
                                    dstSRS='EPSG:4326',
                                    resampleAlg='near',  # 或其他适合你数据的重采样方法
                                    outputType=dataset.GetRasterBand(1).DataType,
                                    xRes=0.000766515,  # 分辨率对应的大概经纬度分辨率，视具体情况调整，可以去QGIS进行转化来参考
                                    yRes=0.000269796)

...
minio_client = Minio(
    ...
    secure=False  # 设置为True以启用安全连接（https）
)
# 指定MinIO存储桶的信息
source_bucket_name = "oge"
target_bucket_name = "oge-cube"
    ...
    with rasterio.Env(**config):
        with ExitStack() as ctx:
            if isinstance(source, (DatasetReader, DatasetWriter, WarpedVRT)):
                ...
                # 从MinIO下载数据
                response = minio_client.get_object(source_bucket_name, source)
                data = response.read()
                src_dst = MemoryFile(data).open()
            ...
            with WarpedVRT(src_dst, **vrt_params) as vrt_dst:
                ...
                if use_cog_driver:
                    ...
                    with MemoryFile() as memfile:
                        with memfile.open(driver='GTiff', width=tmp_dst.width, height=tmp_dst.height,
                                          count=tmp_dst.count, dtype=tmp_dst.dtypes[0], crs=tmp_dst.crs,
                                          transform=tmp_dst.transform, nodata=tmp_dst.nodata) as mem_dst:
                            # Assuming tmp_dst is your DatasetWriter object
                            mem_dst.write(tmp_dst.read())  # Write the content of tmp_dst to mem_dst

                        minio_client.put_object(target_bucket_name, dst_path, memfile,
                                                length=memfile.getbuffer().nbytes)
